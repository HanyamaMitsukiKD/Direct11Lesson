...
public:
	...
	// シェーダーマネージャ
	const KdShaderManager& GetShaderManager();
	KdShaderManager& WorkShaderManager();
	...
	// スクリーン座標から3D座標への変換
	void ClientToWorld(const POINT& screenPos, float porjZ, Math::Vector3& dst);
	void WorldToClient(const Math::Vector3& srcWorld, POINT& dst);
...
private:
	...
	std::unique_ptr<KdShaderManager>	m_pShadermanager = nullptr;
...
public:
	static KdDirect3D &WorkInstance() {
...
#define D3D KdDirect3D::WorkInstance()
