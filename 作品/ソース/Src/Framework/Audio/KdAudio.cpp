...
void KdAudioManager::AudioAsset(std::string_view failPas, std::shared_ptr<KdSoundInstance>& se)
{
	std::shared_ptr<KdSoundEffect> soundData = GetSound(failPas);

	if (!soundData) { return; }

	se = std::make_shared<KdSoundInstance>(soundData);
...
std::shared_ptr<KdSoundEffect> KdAudioManager::GetSound(std::string_view fileName)
{
	...
	if (itFound == m_soundMap.end())
	{
		...
		if (!newSound->Load(fileName.data(), m_audioEng))
		{
			// 読み込み失敗時は、nullを返す
			return nullptr;
		}
		// リスト(map)に登録
		m_soundMap[fileName.data()] = newSound;
